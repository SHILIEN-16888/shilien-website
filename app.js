...
window.SHILIEN={"company_zh": "仕聯", "company_en": "SHILIEN", "phone_display": "04-2337-2669", "phone_raw": "0423372669", "email": "admin@shilien.com", "line_url": "https://line.me/R/ti/p/@029kpqds", "address": "台中市烏日區三榮路一段350號", "hours": "08:00–18:00", "service_area": "台中市", "map_url": "https://www.google.com/maps/search/?api=1&query=台中市烏日區三榮路一段350號"};
...
function copyForm(form, title, fields){
  ...
  const text=[title,...fields.map(([label,key])=>label+"："+(d.get(key)||"無"))].join("\n");
  navigator.clipboard.writeText(text).then(()=>alert("資料已整理並複製，可直接貼到 LINE 傳送。"));
}
const pf=document.querySelector("#partnerForm");
if(pf)pf.addEventListener("submit",e=>{e.preventDefault();copyForm(pf,"【SHILIEN 企業合作洽詢】",[["機構名稱","org"],["聯絡人","name"],["職稱","title"],["電話","phone"],["Email","email"],["合作類型","type"],["預估每月接送量","volume"],["希望開始時間","start"],["需求說明","message"]])});
const bf=document.querySelector("#bookingForm");
if(bf)bf.addEventListener("submit",e=>{e.preventDefault();copyForm(bf,"【SHILIEN 接送預約】",[["聯絡人","name"],["電話","phone"],["服務","service"],["日期","date"],["時間","time"],["上車","pickup"],["目的地","destination"],["輪椅","wheelchair"],["回程","return_trip"],["備註","message"]])});
